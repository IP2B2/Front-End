...
export const getAccessRequests = async () => {
    ...
    try {
        ...
        return {
            ...
            payload: error.response?.data?.error || 'An error occurred while fetching access requests'
        };
    }
};
...
export const getEnrichedAccessRequests = async () => {
    ...
    const enrichedAccessRequests = 
        await Promise.all(
            accessRequests?.payload?.map(async (request) => {
                ...
                let user;
                try { 
                    user = await axios.get(process.env.BACKEND_URI + '/accessRequests/' + request.id + '/user', {
                        headers: {
                            'Authorization': `Bearer ${token}`,
                            'Content-Type': 'application/json'
                        },
                ...
                let laboratory = await getLaboratoryById(request.equipment?.laboratoryId);
                request.equipment.laboratory = laboratory.success ?  laboratory.payload : null;
                return request;
            })
        );
    ...
    if(enrichedAccessRequests.some(req => !req.equipment || !req.user)) {
        return {
            success: false,
            status: 500,
            payload: "Eroare la obtinerea echipamentelor sau utilizatorilor asociati cererilor de acces"
        };
    }
    return {
        success: true,
...
export const serviceApproveAccessRequest = async (accessRequestId) => {
    ...
    try {
        ...
        const response = await axios.patch(process.env.BACKEND_URI + '/access-requests/' + accessRequestId, {
            status: 'APPROVED'
        }, {
            headers: {
                'Authorization': `Bearer ${token}`,
        ...
        return {
            success: false,
            status: error.response?.status || 500,
            payload: error.response?.data?.error || 'An error occurred while approving the access request'
        };
    }
};
...
export const serviceRejectAccessRequest = async (accessRequestId) => {
    ...
    try {
        ...
        console.log("serviceRejectAccessRequest - accessRequestId:", accessRequestId);
        const response = await axios.patch(process.env.BACKEND_URI + '/access-requests/' + accessRequestId, {
            status: 'REJECTED'
        }, {
            headers: {
        ...
        return {
            success: false,
            status: error.response?.status || 500,
            payload: error.response?.data?.error || 'An error occurred while rejecting the access request'
        };
    }
};
