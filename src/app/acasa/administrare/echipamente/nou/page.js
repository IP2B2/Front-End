...
export default function AdminAdaugareEchipament() {
  ...
  const handleCreateEquipment = async () => {
    ...
    try {
      const equipmentData = {
        name: name.trim(),
        inventoryNumber: parseInt(inventoryNumber.trim()),
        availabilityStatus: 'AVAILABLE',
        laboratoryId: 2, 
        description: description.trim(),
        usage: usage.trim(),
        material: material.trim(),
        photo: JSON.stringify (imageLinks.length > 0 ? imageLinks : []),
      ...
      if (response.success) {
        ...
        console.error('Eroare la crearea echipamentului:', response);
        setIsError(true);
      ...
      setIsError(true);
      setErrorMessage(error.message || 'A apărut o eroare la crearea echipamentului.');
    } finally {
  ...
  return (
    <div className={stylesNew.container}>
      <ProductAddedSucc open={formSuccess} onClose={() => {
        redirect('/acasa/administrare/echipamente');
      }} />
      <div className={styles.backButtonWrapper}>
        <BackArrow backTo="/acasa/administrare/echipamente" />
      </div>
      <div className={stylesNew.formCard}>
        <DefaultFormLayout
          ...
          <FormContainer>
            ...
            <FormField 
              type={"checkbox"}
              label={"Este echipament complex."}
              value={isComplex}
              setState={setIsComplex}
            />
            <FormButton
