...
import { getEquipmentById } from "@/lib/service/EquipmentService";
import { getAccessRequestsByEquipmentId } from "@/lib/service/AccessRequestService";
...
export default function EchipamentPage() {
	...
	useEffect(() => {
		async function fetchEquipment() {
			...
			const data = await getEquipmentById(authToken, equipmentId);
			setEquipment(data);
			...
			<div className={styles.descriptionContainer}>
				...
				<div className={styles.buttonGroup}>
					...
					>
						...
						{showCalendar && <AbsoluteCalendar ref={calRef} />}
					</button>
					<button
						className={`${styles.actionButton} ${Inter500.className}`}
						onClick={() => router.push(`/home/echipamente/echipament/${equipmentId}/inchiriere`)}
					>
						Închiriază
					</button>
				</div>
...
export const AbsoluteCalendar = ({ children, notAbsolute }) => {
	const {
		selectedDay,
		setSelectedDay,
		selectedMonth,
    ...
    useEffect(() => {
        ...
        const calendarDays = newDays.map(day => {
            const isUnavailable = unavailableDates.some(unavailableDay => 
                unavailableDay.dateObject.toDateString() === day.dateObject.toDateString()
            );
            return {
                ...day,
                unavailable: isUnavailable,
                selected: selectedDay && day.dateObject.toDateString() === selectedDay.toDateString()
            };
        });
	...
	return (
		<div className={`${calStyles.absoluteCalendar} ${notAbsolute ? calStyles.notAbsolute : ''}`}>
            ...
            <div className={calStyles.calendarWeekdays + ' ' + (days.length > 35 ? calStyles.calendarWeekdaysLong : '')}>
             ...
             <div className={calStyles.calendarBody + ' ' + (days.length > 35 ? calStyles.calendarBodyLong : '')}>
                {days.map((day, idx) => (
                    <div
                        key={idx}
                        className={`${calStyles.calendarDay} ${day.currentMonth ? calStyles.currentMonthDay : ''} ${day.unavailable ? calStyles.busyDay : ''}`}
                    >
                        {day.date}
                    </div>
                ))}
...
function getCalendarDays(year, month) {
    return [...getPrevMonthDaysToMonday(year, month).map(day => ({
